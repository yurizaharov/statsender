...
const methods = {
    ...
    onStartApp: async function (name: string) {
    ...
    getStatsData: async function () {
        ...
        for (const partner of initialData) {
            const sqlData: any = await oracle.sqlRequest(partner);
    ...
    sendStatsData: async function () {
        const stringLength = [21, 9, 9];
        const initialData = await functions.getInitialData();
        const [dateToRead, quartersToRead]: any = functions.datesToRead();
